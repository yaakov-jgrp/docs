...
## Subagent finding (UNVERIFIED)

Walked scans **~60–136**. **No dedicated marriage alphabet** found in the expected slot (after last marriage act, before deaths at ~113).

Possible explanations (all open until headers are read):

1. **1872 book lacks a separate marriage alphabet** (clerk omitted it).
2. Alphabet exists but is **misplaced** or uses an unexpected header (*протоколъ* only, consolidated index, etc.).
3. Subagent **mis-labelled** a page (birth/death index read as marriage block) — same failure mode as 1868–1871 downloader passes.
